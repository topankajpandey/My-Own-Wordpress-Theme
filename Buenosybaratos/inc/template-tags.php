 ...
 */
function buenosybaratos_excerpt_more( $more ) {
        $continue_reading = (!empty(get_theme_mod("search_continue_reading")))?get_theme_mod("search_continue_reading"):'Continue reading';
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( __( $continue_reading, 'buenosybaratos' ), '' )
		);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'buenosybaratos_excerpt_more' );
endif;
